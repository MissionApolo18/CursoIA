            ...
            if nodo.get_depth() < prof_max:
                ...
                for child in nodo.expand():
                    ...
                    if stop(child):
                        ...
                        # agregamos los sucesores a la agenda
                        agenda.append(child)
                        # marcamos el estado como explorado
                        explored.add(child)
        ...
        while Df and Db:
            ...
            for n in Db:
                ...
                for h in n.expand():
                    if h in Df:
                        print(s)
                        # construir la trayectoria desde el inicio hasta el nodo de intersección
                        r1 = trajectory(Df[h])
                        # construir la trayectoria desde el nodo de intersección hasta la meta
                        r2 = trajectory(Db[h])
                        r2.reverse()
                        # concatenar las dos trayectorias
                        return r1 + r2[1:]
                    if h not in E:
                        # conserva esta línea es parte de la salida
                        s += '-'
